...
class ImportFcstd(object):
    ...
    def handle__AppLinkElement(self, func_data):
        ...
        if obj_label not in bpy.data.objects:
            ...
            result_bobj = bpy.data.objects[linkedobj_label]
            # fix name (from autogenerated to correct)
            result_bobj.name = obj_label
        ...
        if self.config["update"] or flag_new:
            ...
            # here is a 'hack'
            # we need to also apply the placement offset
            # from the 'array' object itself
            print(pre_line + "obj_parent.Placement.Base", obj_parent.Placement.Base)
